...
def test_read_package():
    ...
    assert callable(homework.read_package), "`read_package` должна быть функцией."
    assert isinstance(
...
def test_InfoMessage():
    assert inspect.isclass(homework.InfoMessage), "`InfoMessage` должен быть классом."
    ...
    for p in ["training_type", "duration", "distance", "speed", "calories"]:
        assert p in info_message_signature_list, (
            "У метода `__init__` класса `InfoMessage` должен быть " f"параметр {p}."
        )
...
def test_Training():
    assert inspect.isclass(homework.Training), "`Training` должен быть классом."
    for name, value in {
        "LEN_STEP": ValueInfo(
            0.65,
            (
        ...
        "M_IN_KM": ValueInfo(1000, (" для перевода значений из метров в километры")),
    }.items():
    ...
    for param in ["action", "duration", "weight"]:
        assert param in training_signature_list, (
            "У метода `__init__` класса `Training` должен быть " f" параметр {param}."
        )
...
def test_Training_get_distance(input_data, expected):
    ...
    assert result == expected, "Проверьте формулу подсчета дистанции класса `Training`"
...
def test_Training_get_mean_speed(input_data, expected):
    ...
    assert result == expected, (
        "Проверьте формулу подсчета средней скорости движения " "класса `Training`"
    )
...
def test_Training_show_training_info(monkeypatch):
    ...
    monkeypatch.setattr(training, "get_spent_calories", mock_get_spent_calories)
...
def test_Swimming():
    ...
    for value_info in {
        "CALORIES_MEAN_SPEED_SHIFT": ValueInfo(
            1.1, (" для смещения значения средней скорости")
        ),
        "CALORIES_WEIGHT_MULTIPLIER": ValueInfo(2, " для множителя скорости"),
    }.values():
    ...
    for param in ["action", "duration", "weight", "length_pool", "count_pool"]:
        assert param in swimming_signature_list, (
            "У метода `__init__` класса `Swimming` " f" должен быть параметр {param}."
        )
...
def test_SportsWalking():
    ...
    for value_info in {
        "CALORIES_WEIGHT_MULTIPLIER": ValueInfo(
            0.035, " для множителя веса спортсмена"
        ),
        "CALORIES_SPEED_HEIGHT_MULTIPLIER": ValueInfo(
            0.029,
            (" для множителя частного квадрата средней скорости и " "роста спортсмена"),
        ),
        "KMH_IN_MSEC": ValueInfo(0.278, " для перевода значений из км/ч в м/с"),
        "CM_IN_M": ValueInfo(100, " для перевода значений из сантиметров в метры"),
    }.values():
...
def test_Running():
    ...
    for value_info in {
        "CALORIES_MEAN_SPEED_MULTIPLIER": ValueInfo(
            18, " для множителя средней скорости"
        ),
        "CALORIES_MEAN_SPEED_SHIFT": ValueInfo(1.79, " для сдвига средней скорости"),
    }.values():
...
def test_main():
    ...
    assert isinstance(homework.main, types.FunctionType), "`main` должна быть функцией."
